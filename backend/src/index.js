...
io.on("connection", async (socket) => {
    ...
    if (playerId) {
      ...
      if (playerData && command.type === "move") {
        ...
        if (result.success) {
          ...
          socket
            .to(`room_${newRoomId}`)
            .emit("playerJoined", { playerId, roomId: newRoomId });
          ...
          const roomDescription = await roomIdAndPlayerIdToRoomDescription(
            newRoomId,
            playerId
          );
      ...
      if (playerData && command.type === "examine") {
        ...
        try {
          ...
          if (inventoryPartRes.rows.length > 0) {
            ...
            socket.emit(
              "command",
              `You look at the ${input} of the ${itemName}: ${partDescription}`
            );
            return;
          }
          ...
          socket.emit(
            "command",
            `You don't see anything with a ${input} to examine.`
          );
        } catch (error) {
      ...
      if (playerData && command === "look") {
        ...
        try {
          ...
          const roomRes = await pool.query(
            "SELECT detailed_description FROM rooms WHERE id = $1",
            [roomId]
          );
          ...
          if (roomRes.rows.length > 0 && roomRes.rows[0].detailed_description) {
            ...
            socket.emit(
              "command",
              `You look around but don't notice anything unusual.`
            );
          }
        } catch (error) {
          ...
          socket.emit(
            "command",
            "There was an error trying to look around. Please try again later."
          );
        }
      }
      ...
      if (playerData && command.type === "inventory") {
        try {
          ...
          const inventoryRes = await pool.query(
            "SELECT name FROM items WHERE held_by_player_id = $1",
            [playerData.playerId]
          );
          ...
          socket.emit(
            "command",
            "There was an error fetching your inventory. Please try again later."
          );
        }
      }
      ...
      if (playerData && command.type === "pickup") {
        ...
        try {
          ...
          const itemRes = await pool.query(
            "SELECT * FROM items WHERE lower(name) = $1 AND current_room_id = $2",
            [itemName, roomId]
          );
          ...
          if (itemRes.rows.length > 0) {
            ...
            await pool.query(
              "UPDATE items SET held_by_player_id = $1, current_room_id = NULL WHERE id = $2",
              [playerData.playerId, item.id]
            );
            await pool.query(
              "INSERT INTO player_inventory (player_id, item_id) SELECT $1, id FROM items WHERE lower(name) = $2",
              [playerData.playerId, command.itemName]
            );
            ...
            const updatedRoomDescription =
              await roomIdAndPlayerIdToRoomDescription(
                roomId,
                playerData.playerId
              );
            io.to(`room_${roomId}`).emit(
              "updateRoomDescription",
              updatedRoomDescription
            );
          } else {
          ...
          socket.emit(
            "command",
            "There was an error trying to pick up the item. Please try again later."
          );
        }
      }
      ...
      if (playerData && command.type === "drop") {
        ...
        try {
          ...
          const itemRes = await pool.query(
            "SELECT * FROM items WHERE lower(name) = $1 AND held_by_player_id = $2",
            [itemName, playerData.playerId]
          );
          ...
          if (itemRes.rows.length > 0) {
            ...
            await pool.query(
              "DELETE FROM player_inventory WHERE player_id = $1 AND item_id = (SELECT id FROM items WHERE lower(name) = $2)",
              [playerData.playerId, command.itemName]
            );
            await pool.query(
              "UPDATE items SET held_by_player_id = NULL, current_room_id = $1 WHERE id = $2",
              [roomId, item.id]
            );
            playerData.inventory = playerData.inventory.filter(
              (item) => item !== command.itemName
            );
          ...
          socket.emit(
            "command",
            "There was an error trying to drop the item. Please try again later."
          );
        }
      }
    }
  ...
  socket.on("joinGame", async (playerId) => {
    ...
    const res = await pool.query("SELECT * FROM players WHERE id = $1", [
      playerId,
    ]);
    ...
    if (player) {
      ...
      socket
        .to(`room_${player.current_room_id}`)
        .emit("playerJoined", { playerId, roomId: player.current_room_id });
    } else {
  ...
  socket.on("disconnect", async () => {
    ...
    if (playerData) {
      try {
        ...
        await pool.query(
          "UPDATE players SET current_room_id = $1, health = $2, level = $3 WHERE id = $4",
          [
            playerData.roomId,
            playerData.health,
            playerData.level,
        ...
        await pool.query("DELETE FROM player_inventory WHERE player_id = $1", [
          playerData.playerId,
        ]);
        for (let itemName of playerData.inventory) {
          ...
          await pool.query(
            ...
            [playerData.playerId, itemRes.rows[0].id]
          );
        }
      } catch (error) {
